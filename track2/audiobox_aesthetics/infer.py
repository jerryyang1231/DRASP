...
class AesPredictor:
    ...
    def setup_model(self):
        ...
        if self.checkpoint_pth is not None:
            ...
            # 如果裡面沒有 model_cfg，就自動去同一資料夾讀 config.json
            if "model_cfg" not in ckpt:

                cfg_path = os.path.join(os.path.dirname(checkpoint_file), "config.json")
                cfg      = json.load(open(cfg_path))
                ckpt["model_cfg"]       = cfg
                ckpt["target_transform"]= cfg["target_transform"]
            ...
            # add transforms & pooling args
            cfg["target_transform"] = ckpt["target_transform"]
            cfg["pooling_type"] = self.pooling_type
            cfg["segment_size"] = self.segment_size

            model = AesMultiOutput(**cfg)
...
def initialize_predictor(ckpt=None, pooling_type="tap", segment_size=1):
    model_predictor = AesPredictor(checkpoint_pth=ckpt,
                                    data_col="path",
